...
# Layer 1：類別衝突判定
CONFIDENCE_GAP_HARD    = 0.5          # 衝突代理人間信心差 > 此值 → HARD
                                       # 從 0.4 提至 0.5：pilot_v7 觀察 HARD 觸發
                                       # 從 v6 的 9 次激增為 16 次，多為 MIL 低信心預測
                                       # 與 VLM 高信心預測的 false-HARD（Fix E1 副作用）
CONFIDENCE_SOFT_MAX    = 0.7          # 信心 < 此值 + 無直接矛盾 → SOFT
                                       # 從 0.6 提升至 0.7：pilot v2 觀察到 SOFT 觸發 0/56
                                       # （門檻太嚴）；錯誤分類平均信心 0.67，0.7 門檻
                                       # 能涵蓋多數 borderline cases 觸發 refine

# Layer 2：信心門檻（待 pilot experiment 確認）
# 做法：跑 20-30 個 UCF-Crime 標準案例，
#        取 Action Agent 正確分類時信心的第 75 百分位數作為 HARD 門檻。
CONFIDENCE_HARD_THRESHOLD = 0.96      # HARD 觸發最低信心（pilot_v6 正確分類 p75）
CONFIDENCE_SOFT_THRESHOLD = 0.81      # SOFT 觸發最低信心（pilot_v6 正確分類 p50）
...
# escalation_head 當前由 train_mil.py 之 per-video ranking loss + sparsity term
# 主導而收斂至 near-zero（pilot_v3 觀察：所有類別 escalation ≈ 0）。
# 設為 False 時，L2 HARD 的 escalation 路徑被停用，避免對任何高信心
# HIGH_SEVERITY 預測都誤觸發 re-analyze。
# 若未來以 per-snippet BCE 重訓 escalation_head，改 True 即可重啟。
ESCALATION_HEAD_TRUSTED = False
...
class ReflectorAgent:
    ...
    def _layer1_temporal(
        ...
        # HARD ④：MIL vs VLM 類別不一致（Fix E1, 2026-04-24，refined 2026-04-25）
        # ActionEmotion Agent 同時保留 MIL 原始預測（metadata）與 VLM 最終預測
        # （crime_category）。若兩者類別不同 + 均為非 Normal + VLM 與 MIL 信心差
        # > CONFIDENCE_GAP_HARD → 視為雙意見顯著衝突，觸發 HARD 重分析。
        #
        # MIL_CONF_MIN 守衛（2026-04-25）：pilot_v7 觀察到許多 false-HARD 來自
        # MIL conf 低於 0.4 的預測（屬於 MIL 「無意見」的雜訊），不該作為觸發
        # HARD 的有效第二意見。要求 mil_conf >= MIL_CONF_MIN 才視為可信對照。
        MIL_CONF_MIN = 0.4
        if ae_report:
            mil_cat  = ae_report.metadata.get("mil_crime_type")
            mil_conf = ae_report.metadata.get("mil_confidence", 0.0)
            vlm_used = ae_report.metadata.get("vlm_used", False)
            if (vlm_used
                    and mil_cat is not None
                    and mil_cat not in ("Normal", "")
                    and ae_report.crime_category not in ("Normal", "")
                    and mil_cat != ae_report.crime_category
                    and mil_conf >= MIL_CONF_MIN
                    and abs(ae_report.confidence - mil_conf) > CONFIDENCE_GAP_HARD):
                is_hard = True
                conflicts.append(ConflictRecord(
                    agent_a="MIL Head",
                    agent_b="VLM (Qwen3-VL)",
                    conflict_type="category_mismatch",
                    description=(
                        f"MIL 預測 {mil_cat}（{mil_conf:.2f}）vs "
                        f"VLM 預測 {ae_report.crime_category}（{ae_report.confidence:.2f}），"
                        f"信心差 {abs(ae_report.confidence - mil_conf):.2f} > "
                        f"{CONFIDENCE_GAP_HARD} → 雙意見顯著衝突（L1 HARD）"
                    ),
                    severity=0.85,
    ...
    def _layer2_causal(
        ...
        # 守衛：escalation_head 未訓練好時停用，避免誤觸發
        if ae_report and ESCALATION_HEAD_TRUSTED:
        ...
        # ── SOFT：RAG 法律要件匹配率過低（Fix E2, 2026-04-24）──
        # 若 VLM 分類為犯罪類且信心 > 0.7，但 RAG 要件檢核匹配率 < 0.3，
        # 代表畫面中幾乎沒有該罪名的法定要件 → 強烈懷疑分類錯誤 → SOFT refine。
        # 不強換類別（避免 pilot_v3 Shooting033 類型錯換），由 refine 給 VLM 提示。
        RAG_ELEMENT_MATCH_SOFT_THRESHOLD = 0.3
        for report in reports:
            if report.crime_category == "ENVIRONMENTAL_ASSESSMENT":
                continue
            ratio = report.metadata.get("rag_element_match_ratio")
            if ratio is None:
                continue
            if ratio < RAG_ELEMENT_MATCH_SOFT_THRESHOLD and report.confidence > 0.7:
                em_raw = report.metadata.get("rag_element_match", (0, 0))
                conflicts.append(ConflictRecord(
                    agent_a=report.agent_name,
                    agent_b="LegalElementCheck",
                    conflict_type="element_mismatch",
                    description=(
                        f"{report.agent_name} 判定 {report.crime_category}"
                        f"（信心 {report.confidence:.2f}），但 RAG 要件檢核"
                        f" {em_raw[0]}/{em_raw[1]} 匹配（比率 {ratio:.2f}）< "
                        f"{RAG_ELEMENT_MATCH_SOFT_THRESHOLD} → 畫面缺乏該罪名法定"
                        "要件 → CP SOFT，建議複檢分類"
                    ),
                    severity=0.50,
                    frames_a=report.frame_references[:3],
    ...
    def _compute_consistency_score(
        ...
        def _mass(report: AgentReport, is_env: bool = False) -> Dict[str, float]:
            ...
            if is_env:
                ...
                # Fix E2：RAG 法律要件匹配率校準（僅對 ActionEmotion，不對 Env）
                # match=1.0 → 乘 1.0（無衰減），match=0.0 → 乘 0.5（強烈下修 crime belief）
                # 無 rag_element_match_ratio 時維持原行為（乘 1.0）
                em_ratio = report.metadata.get("rag_element_match_ratio")
                if em_ratio is not None:
                    calibration = 0.5 + 0.5 * em_ratio
                    m_crime *= calibration
                m_unc   = u
                m_no    = max(0.0, 1.0 - m_crime - m_unc)
